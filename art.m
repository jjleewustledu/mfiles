...
if(~isempty(sess_file)) % read config
    [num_sess,global_type_flag,drop_flag,motionFileType,motion_threshold,global_threshold,use_diff_motion,use_diff_global,use_norms,SPMfile,mask_file,P,M] = ...
        read_art_sess_file(sess_file);
    realignfile = 1;
    %% END ohinds 2008-04-23: config file loading
else
    motion_threshold=[];global_threshold=[];SPMfile=[];mask_file=[];
...
datafiles={};
for i=1:length(P),if ~isempty(P{i}),datafiles{i}=fliplr(deblank(fliplr(deblank(P{i}(1,:)))));else,datafiles{i}=[];end;end; % <alfnie>: keep filenames of functional data (first scan per session only)
...
else   z_Callback(hObject, eventdata, handles,1.0);mv_Callback(hObject, eventdata, handles,1.0);rt_Callback(hObject, eventdata, handles,1.0);
end
...
if (get(handles.showDesign,'Value') == get(handles.showDesign,'Max'))
    ...
    if cur_sess_start-1~=size(design,1),
        ...
        outliers_per_condition=[length(out_idx),sum(abs(design(out_idx,:)>0),1)];
    end
    if length(outliers_per_condition)==length(names)+1,
        legendnames={['Total :',num2str(outliers_per_condition(1)),' outliers']};
        for i=1:length(names), legendnames{i+1}=[names{i},' :',num2str(outliers_per_condition(i+1),'%0.0f'),' outliers (',num2str(100*outliers_per_condition(i+1)/max(eps,outliers_per_condition(1)),'%0.0f'),'%)']; end
        [nill,h2]=legend(h,legendnames{:});%set(h2(1),'visible','off');
    end
    analyses=getappdata(handles.savefile,'analyses');
    analyses.outliers.condition_effects=outliers_per_condition(2:end);
    ...
    if(~isempty(stats_file_outliers))
        for fpidx = 1:2,
            if fpidx==1,
                ...
                fprintf('saving oulier statistics to %s\n',[stats_file_outliers]);
            end
            if(fp ~= -1)
                fprintf(fp,'%10s ','Total');
                fprintf(fp,'%10s ',names{:});
                fprintf(fp,'\n');
                fprintf(fp,'%10.4f ',outliers_per_condition);
                fprintf(fp,'\n');
            end
...
for n1=setdiff(1:length(plotdata.maskscan),out_idx),b(plotdata.maskscan{n1})=0;end
V=plotdata.VY1;
V.fname='art_mask.img';
V.dt=[spm_type('uint8') spm_platform('bigend')];
spm_write_vol(V,b);
disp(['New analysis mask saved to ',fullfile(pwd,V.fname)]);
...
function [num_sess,global_type_flag,drop_flag,motionFileType,motion_threshold,global_threshold,use_diff_motion,use_diff_global,use_norms,SPMfile,mask_file,P,M] = read_art_sess_file(sess_file)
