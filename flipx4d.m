function image_out = flipx4d(image_in)

%FLIPX4D flips input images from x>0 to x<0;
%        input images may be a double or dip_image object
%
%   Usage:  image_out = flipx4d(image_in)
%
%   Author:  John J. Lee 
...
if (~((1 <= ranks(2)) & (ranks(2) <= 4)))
    error('flipx4d only supports objects of rank 1 - 4'); end
...
switch ranks(2)
    ...
    case 4
        dip4rank = image_in;
    otherwise
        error('flipx4d.switch only supports dip_image objects of rank 1 - 4');
end
...
for m = 0:sizes(4)-1
    for l = 0:sizes(3)-1
        ...
        for k = 1:sizes(2)
            for j = 1:sizes(1)
                buffer2(j, sizes(2)+1-k) = buffer(j, k);
            end
        end
