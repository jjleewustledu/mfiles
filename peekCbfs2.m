...
%          peekCbfs2(pid, slice1, sl2, hoFile, mlemTime, osvdTime, ssvdTime, kindRoi, kindImg)
%
...
function [cbfMlem, cbfOsvd, cbfSsvd, FBayes, ho1, ho1gauss,...
          ...
          peekCbfs2(pid, sl1, sl2, hoFile, mlemTime, osvdTime, ssvdTime, rois, kindImg, overlayRois)
...
if (overlayRois)
    ...
    [grepy_, basalp_, whitep_, roisp_] = peekRois(pid, 'allrois', 'dip', 0, 0)
    rois = rois_;
end
...
%%%        end
        ...
        name = [datapath 'cbf_MLEM.4dfp.img'];
	    cbfMlem = read4d(name,'ieee-be','single',lenx,leny,lenz,1,0,0,0);            
        disp(['peekCbfs found ' name]);
    catch
        try
	            timeSig = mlemTimes(pid, 'cbfMlem');
	            name   = [datapath 'SHIMONY_CBF_MLEM_LOGFRACTAL_VONKEN_NO_FILTER_time' timeSig '.4dfp.img'];  
	            theImg = read4d(name,'ieee-be','single',lenx,leny,lenz,1,0,0,0);
				disp(['peekCbfs found ' name]);
	    catch
