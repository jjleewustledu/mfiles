...
function theValues = peekRoiCounts()
    ...
    for p = 1:NPID
        ...
        [grey, basal, white, allRois] = peekRois(p, kindRoi, 'dip', 0, 0);
        ...
        for r = 1:NROI
            for s = 1:NSIDE        
                idx = (p - 1)*NROI*NSIDE + (r - 1)*NSIDE + s;
                if (blur)
					sigmas = fwhh*[1/(2*XY_MMPIXEL) 1/(2*XY_MMPIXEL) 1/(2*Z_MMPIXEL)];
