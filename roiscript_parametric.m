...
rph = readcollection('t:\perfusion\asnr3\pt3\Parametric\mle_rph_net.dat','double',512,1024,1);
ttp = readcollection('t:\perfusion\asnr3\pt3\Parametric\mle_ttp_net.dat','double',512,1024,1);
...
study = newim(1024, 512, Nstudies + 1);
study(:,:,1) = scrubNaNs(rph(:,:,0));
study(:,:,2) = scrubNaNs(ttp(:,:,0));
study(:,:,3) = cbfpet(:,:,0);
study(:,:,4) = mttpet(:,:,0);

tmp = newim(1024, 512, 1);
excel_lhs_mymean = zeros(15, Nstudies);
excel_lhs_mystd = zeros(15, Nstudies);
excel_rhs_mymean = zeros(15, Nstudies);
excel_rhs_mystd = zeros(15, Nstudies);
...
petabscissas = zeros(2*Nrois, PETstudies);
petabscissas(1:15,1) = excel_lhs_mymean(:,3);
petabscissas(16:30,1) = excel_rhs_mymean(:,3);
petabscissas(1:15,2) = excel_lhs_mymean(:,4);
petabscissas(16:30,2) = excel_rhs_mymean(:,4);

mrordinates = zeros(2*Nrois, MRstudies);
