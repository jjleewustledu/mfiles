function parallelSubmitIntelMPIFcn(scheduler, job, props, ...
    clusterHost, remoteDataLocation,procsPerNode,walltime,nodeFlag,email)
...
% $Revision: 1225 $  $Date: 2012-08-23 16:04:28 -0500 (Thu, 23 Aug 2012) $
...
scriptName = 'ParallelIntelMPIWrapper.sh';
% The wrapper script is in the same directory as this file
mpiLibConfScriptName = 'mpiLibConf.m';
dirpart = fileparts(mfilename('fullpath'));
localScript = fullfile(dirpart, scriptName);
mpiLibConfScript = fullfile( dirpart, mpiLibConfScriptName);
% Copy the local wrapper script to the job directory
copyfile(localScript, localJobDirectory);
copyfile(mpiLibConfScript, localJobDirectory);
...
%additionalSubmitArgs = sprintf('-l nodes=%d:%s:ppn=%d -lwalltime=%s -m e -M %s -q matlab', numberOfNodes,nodeFlag, procsPerNode,walltime,email);
if (props.NumberOfTasks < procsPerNode)
additionalSubmitArgs = sprintf('-q matlab -l nodes=%d:ppn=%d,walltime=4:00:00 -W x=GRES:MATLAB_Distrib_Comp_Engine+%d', numberOfNodes, props.NumberOfTasks, props.NumberOfTasks);
dctSchedulerMessage(4, '%s: Requesting %d nodes with %d processors per node', currFilename, ...
    numberOfNodes, props.NumberOfTasks);
else
additionalSubmitArgs = sprintf('-q matlab -l nodes=%d:ppn=%d,walltime=4:00:00 -W x=GRES:MATLAB_Distrib_Comp_Engine+%d', numberOfNodes, procsPerNode, props.NumberOfTasks);
dctSchedulerMessage(4, '%s: Requesting %d nodes with %d processors per node', currFilename, ...
